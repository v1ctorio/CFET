...
fun SingInScreen(navController: NavController) {
    ...
    if (sharedPref.contains("zoneId") && sharedPref.contains("APIKey") && sharedPref.contains("userId") && sharedPref.contains(
            "domain"
        )
    ) {
    ...
    val API_KEY_GENERATOR_ENDPOINT =
    ...
    Scaffold(
        ...
        content = { cpadding ->
            ...
            )
            {
                ...
                ElevatedButton(
                    onClick = { uriHandler.openUri(API_KEY_GENERATOR_ENDPOINT) },
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(8.dp),
